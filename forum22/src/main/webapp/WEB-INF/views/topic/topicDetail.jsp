...
<div class="container">
    ...
    <div class="box" style="margin-top:20px;">
        <div class="talk-item muted" style="font-size: 12px">
            ${fn:length(replyList)}个回复 <c:if test="${not empty replyList}">| 直到<span id="lastreplytime">${topic.lastreplytime}</span></c:if>
        </div>
...
<script>
    ...
    <c:if test="${not empty sessionScope.curr_user}">
        ...
        $("#lastreplytime").text(moment($("#lastreplytime").text()).format("YYYY年MM月DD日HH:mm:ss"));
        $(".reply").text(function () {
           var time=$(this).text();
